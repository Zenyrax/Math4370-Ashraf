...
// Helper function for accessing a 2d index in a flattened array
long int idx(long int m, long int i, long int j) {
  return m * i + j;
}
...
void vec2d::Write() const {
  ...
  for (long int i=0; i<this->m; i++) {
    for (long int j=0; j<this->n; j++) {
      std::cout << std::setprecision(17) << "  " << this->data[idx(m, i, j)];
    }
    std::cout << std::endl;
  }
...
void vec2d::Write(const char *outfile) const {
  ...
  for (long int i=0; i<this->m; i++) {
    for (long int j=0; j<this->n; j++) {
      fstr << std::setprecision(17) << "  " << this->data[idx(m, i, j)];
    }
    fstr << std::endl;
  }
  fstr << std::endl;
...
void vec2d::LinearSum(double a, const vec2d& y, double b, const vec2d& z) {
  ...
  if (y.m != this->m  ||  z.m != this->m || y.n != this->n  ||  z.n != this->n)
  ...
  // perform operation
  for (long int i=0; i<this->m * this->n; i++) {
