...
// This defines a simple arithmetic 2d vector class
class vec2d {
 ...
 public:
  ...
  // returns m
  long int M() const {return m;};

  // returns n
  long int N() const {return n;};
  
  // returns my overall size
  long int Size() const {return m*n;};
  ...
  // For handling 2d inputs
  double& operator()(long int i, long int j) {return data[i * m + j];}
  double operator()(long int i, long int j) const {return data[i * m + j];}
